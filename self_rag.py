...
"""),
    ...
    def get_more_info(self, strategy: dict, urls: list):
        try:
            ...
            # Create grading prompt specific to strategy analysis
            system_prompt = {
                "role": "system",
                "content": f"""
            ...
            logger.info(f"Graded {len(url_docs)} docs, found {len(graded_docs)} relevant")
            
            # Generate embeddings for graded documents
            embeddings = []
            texts = []
            ...
            # Generate query embedding and find similar documents
            query_embedding = generate_embedding(strategy['title'])
            similarities = []
            ...
            # Generate analysis from relevant content
            raw_analysis = self.summarize_text(
                text="\n".join(doc.page_content for doc in relevant_docs),
                context=strategy['title'],
                strategy=strategy
